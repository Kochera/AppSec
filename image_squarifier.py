...
def generate_new_image(image):
    try:
        ...
        choice = raw_input("Would you like to scaled the image or add padding?\n(Type s for scaled and p for transparent padding): ")
        ...
        if(choice.lower() == "s"):
            ...
            R_Value = raw_input("Enter R Value(0-255): ")
            while(int(R_Value) > 255 or int(R_Value) < 0):
                R_Value = raw_input("Enter R Value(0-255): ")
            G_Value = raw_input("Enter G Value(0-255): ")
            while(int(G_Value) > 255 or int(G_Value) < 0):
                G_Value = raw_input("Enter G Value(0-255): ")
            B_Value = raw_input("Enter B Value(0-255): ")
...
if __name__=="__main__":
    ...
    while(done == False):
        ...
        new_image_name = raw_input("Enter new file name for scaled image: ")
